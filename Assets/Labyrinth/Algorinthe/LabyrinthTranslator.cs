...
namespace UdeS.Promoscience.Generator
{
    public class LabyrinthTranslator
    {
        ...
        public void reverseTranslate(int[] data)
        {
            ...
            {
                if (data[i] >= Constants.TILE_START_START_ID && data[i] <= Constants.TILE_START_END_ID)
                {
                ...
                else if (data[i] >= Constants.TILE_FLOOR_START_ID && data[i] <= Constants.TILE_FLOOR_END_ID) labyrinth.Add(new Road(i / width, i % width));
                else if (data[i] >= Constants.TILE_END_START_ID && data[i] <= Constants.TILE_END_END_ID) labyrinth.Add(new EndRoad(i / width, i % width));
