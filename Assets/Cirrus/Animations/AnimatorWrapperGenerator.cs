...
namespace Cirrus.Animations
{
    ...
    public class AnimatorWrapperGenerator : UnityEngine.ScriptableObject
    {
        ...
        private const string _clipLengthValuesString = "_clipLengthValuesValues";
        ...
        public string GenerateWrapperCode()
        {
            ...
            writer.WriteLine($"float GetClipLength({FormatAnimationEnum(_name)} state);");
            ...
            writer.WriteLine($"private Dictionary<{FormatAnimationEnum(_name)},float> {_clipLengthValuesString} = new Dictionary<{FormatAnimationEnum(_name)},float>();");

            // Play method
            writer.WriteLine($"public void Play({FormatAnimationEnum(_name)} animation, float normalizedTime)");
            writer.BeginBlock();
            writer.WriteLine("_animator.Play((int)animation, -1, normalizedTime);");
            writer.EndBlock();

            writer.WriteLine($"public void Play({FormatAnimationEnum(_name)} animation)");
            writer.BeginBlock();
            writer.WriteLine("_animator.Play((int)animation);");
            writer.EndBlock();
            ...
            foreach (var layer in _animator.layers)
            {
                foreach (var state in layer.stateMachine.states)
                {
                    ...
                    writer.WriteLine($"{_clipLengthValuesString}.Add({FormatAnimationEnum(_name)}.{FormatAnimation(state.state.name)},Cirrus.Animations.Utils.GetClipLength(_animator.runtimeAnimatorController,\"{FormatClip(state.state.name)}\"));");
                }
            }
            writer.EndBlock();
            ...
            writer.BeginBlock();
            writer.WriteLine($"if({_clipLengthValuesString}.TryGetValue(state, out float res)) return res;");
            writer.WriteLine("return -1f;");
            writer.EndBlock();
        ...
        {
            string filePath = Path.GetDirectoryName(UnityEditor.AssetDatabase.GetAssetPath(this));
